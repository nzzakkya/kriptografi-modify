...
if (isset($_REQUEST['encrypt'])) 
	{
		...
		?>
		Input Text : <div style="color:red"><?php echo $input; ?></div>
		Cypher Text : <div style="color:red"><?php echo Encipher($input, $key, '-'); ?></div>
		<?php
	...
	elseif (isset($_REQUEST['decrypt'])) 
	{
	    Decipher($input, $key);
	    ?>
		Input Text : <div style="color:red"><?php echo $output; ?></div>
		Plain Tex : <div style="color:red"><?php echo Decipher($input, $key); ?></div>
		<?php
	...
	function Encipher($input, $key, $padChar)
	{
		...
		$changeChar = str_replace(array_keys($Key2), $Key2, $input);

		for ($i = 0; $i < $totalChars; ++$i)
		{
			$currentRow = $i / $totalColumns;
			$currentColumn = $i % $totalColumns;
			$rowChars[$currentRow][$currentColumn] = $changeChar[$i];
		}
